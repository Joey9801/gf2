...
void Monitor::setValue(unsigned int pointId, unsigned int time, bool value) {
...
void Monitor::renamePoint(unsigned int pointId, std::string newName) {
...
const std::vector<std::pair<unsigned int, bool> >& Monitor::getLog(unsigned int pointId) {
  if(_points.find(pointId) == _points.end()) {
    //TODO a proper error
    throw 1;
  }
